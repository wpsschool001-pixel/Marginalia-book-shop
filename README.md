...
## Accounts (each reader has their own login)

There's no single shared password anymore — anyone visiting the site sees a
**sign-in / create account** screen first. Each person picks their own
username and password (minimum 6 characters), which is stored — as a
scrambled/hashed value, never the plain password itself — in
`data/users.json`.

- New visitors click **"Create an account"** on that first screen
- Returning visitors sign in with what they already picked
- A **"Log out"** button sits in the top-right of the site once signed in

**Nothing to configure** — this works the same locally and once deployed,
with no environment variables needed for login itself.

One limitation worth knowing: sessions (i.e., staying logged in) are kept in
the server's memory, not saved to disk. That means if the server restarts —
which free hosts like Render do automatically after periods of inactivity —
everyone gets signed out and has to log back in. Their account and password
aren't affected, just their "stay logged in" status. This is a fine tradeoff
for a project this size; if you outgrow it, the fix is to store sessions in
`data/sessions.json` the same way `users.json` is stored, instead of in the
`sessions` Map in `server.js`.
...
**7. Click Deploy.** After a minute or two, Render gives you a public URL
like `https://marginalia-bookshop.onrender.com`. Visiting it shows the
sign-in/create-account screen — anyone can create their own account there.
